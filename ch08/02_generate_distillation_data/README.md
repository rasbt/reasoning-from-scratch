...
- [3. Hosted generation with MiniMax](#3-hosted-generation-with-minimax)
  - [3.1 MiniMax setup](#31-minimax-setup)
  - [3.2 Data generation with MiniMax](#32-data-generation-with-minimax)
...
&nbsp;
## 3. Hosted generation with MiniMax

[MiniMax](https://www.minimaxi.com/) provides cloud-hosted LLMs through an OpenAI-compatible API. Their latest model, MiniMax-M2.7, supports a 1M token context window and is well-suited for generating high-quality distillation data.
...
&nbsp;
### 3.2 Data generation with MiniMax

The MiniMax script works similarly to the OpenRouter script:

```bash
MINIMAX_API_KEY="YOUR_API_KEY" uv run generate_with_minimax.py \
  --math_json math_train_sample.json \
  --dataset_size 5 \
  --model MiniMax-M2.7 \
  --num_processes 1 \
